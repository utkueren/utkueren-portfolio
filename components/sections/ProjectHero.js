...
export default function ProjectHero({ project }) {
  ...
  return (
    <section className="relative h-[100svh] bg-black text-white">
      {/* Marquee + Back */}
      ...
      <div className="h-[calc(100svh-72px)]">
        <div className="mx-auto max-w-[90rem] px-6 sm:px-12 lg:px-86 h-full py-8">
          <div className="grid grid-cols-1 lg:grid-cols-[1.25fr_1fr] gap-8 h-full">
            {/* LEFT: media */}
            <div className="relative rounded-2xl border border-white/10 bg-white/5 overflow-hidden h-[48vh] lg:h-full">
            ...
            <div className="flex flex-col rounded-2xl border border-white/10 bg-white/5 p-6 lg:p-8 overflow-hidden">
              ...
              <h1 className="mt-3 text-3xl sm:text-4xl font-semibold tracking-[0.1em] leading-[0.95]">
                {title}
              </h1>

              {!!desc && (
                <p className="mt-4 text-sm leading-relaxed text-white/75 line-clamp-6">
                  {desc}
                </p>
              )}
